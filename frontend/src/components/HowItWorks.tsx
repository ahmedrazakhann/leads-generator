        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-10 md:gap-4 max-w-6xl mx-auto">
          {STEPS.map((step, idx) => (
            <React.Fragment key={step.num}>
              ...
              >
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-6xl font-black text-slate-800/30 group-hover:text-primary/20 transition-colors pointer-events-none">
                  {step.num}
                </div>
                <div className="w-20 h-20 rounded-3xl bg-slate-800 border border-slate-700 flex items-center justify-center text-primary mb-8 mx-auto shadow-xl group-hover:scale-110 group-hover:border-primary/50 transition-all duration-300">
                  {step.icon}
                </div>
